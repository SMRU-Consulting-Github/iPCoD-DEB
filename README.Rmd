...
To download the newest version of the code, check the 'Releases' to the right and then press 'Source code'. A zipped folder with the code and all necessary input files needed to run the code should be automatically uploaded. The folder will also include the Helpfile. 
The iPCoD v5.2 has been widely used as part of assessments of the impact of disturbance on UK cetacean populations. We have therefore ensured that the iPCoD+DEB closely resembles the current version which was informed by expert elicitation (iPCoD 5.2). This should allow users to run both versions of iPCoD with no additional training and with almost identical sets of input values.
Please refer to the Helpfile folder above to see how to install and use the model. Please refer to the licence file to get an overview of who can use the model and how. 
